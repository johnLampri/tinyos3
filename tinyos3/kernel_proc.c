...
sys_ThreadExit(exitval);
  ...
  if(cur_thread()!=NULL){
    ...
    void* args = cur_thread()->ptcb->args;
      ...
      exitval= call(argl,args);
      sys_ThreadExit(exitval);
      //na enimerwsw to exit val sto sys_thread_Exit
  }
}


void refcount_increase(PTCB* ptcb){
  ptcb->refcount=ptcb->refcount+1;

  }

void refcount_decrease(PTCB* ptcb){
  ptcb->refcount=ptcb->refcount-1;
  if(ptcb->refcount==0){
...
*/}
  ...
  Fid_t sys_OpenInfo()
  {
    ...
    procinfocb->cursor=get_pcb(0);
    procinfocb->procinfo=NULL;
  ...
  int procinfo_read(void* pr,char* buf, unsigned int size){
    procinfo_cb* procinfocb=(procinfo_cb*)pr;
    ...
    procinfocb -> procinfo -> pid= get_pid(procinfocb->cursor);
    procinfocb -> procinfo -> ppid=get_pid(procinfocb -> cursor->parent);
    ...
    int args_copied=procinfocb->cursor->argl;

    if(args_copied>=PROCINFO_MAX_ARGS_SIZE){
      args_copied=PROCINFO_MAX_ARGS_SIZE;
    }
    memcpy(procinfocb->procinfo->args, procinfocb->cursor->args, args_copied);
  ...
  int procinfo_close(void* fid){
    procinfo_cb *pr=(procinfo_cb*) fid;
    if(pr==NULL){
      return -1;
    }
    free(pr);
