...
#endif
#define fqueues 10 //here i set the number of queues of kernel_sched
int levelupcounter; //this is a counter for the level up(calls of yield)
...
rlnode SCHED[fqueues]; /* The scheduler queue */
...
static TCB* sched_queue_select(TCB* current)
{
	/* Get the head of the SCHED list */
		int rightpos = fqueues-1; // i need something sure to place in line 344
	for(int i=fqueues-1;i>=0; i--){
		if(!is_rlist_empty(&SCHED[i])){
			rightpos = i; //i check if list is empty if not i place it in line 344
			break;
		}
	}
	...
	/* Get the head of the SCHED list */
	rlnode* sel = rlist_pop_front(&SCHED[rightpos]);
...
void yield(enum SCHED_CAUSE cause){
	levelupcounter++; //each time i call the yield i count for the level up
	...
	if(levelupcounter > 3000){
	  	...
	  	if((current->priority != 0)){
		    ...
		    if(cause ==SCHED_IO){
		     	current->priority = fqueues - 1; //top priority gia IO panta
		    }
